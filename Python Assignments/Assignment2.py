...
class FlipPredictor(object):
    ...
    def pheads(self):
        ...
        for i in range(self.n):
            heads_prob += self.probs[i] * self.coins[i]
        tails_prob = 0
        for i in range(self.n):
            tails_prob += self.probs[i] * (1 - self.coins[i])

        heads_prob /= heads_prob + tails_prob
        return heads_prob

    def update(self, result):
        if result == 'H':
            for i in range(self.n):
                self.probs[i] *= self.coins[i]
        else:
            for i in range(self.n):
                self.probs[i] *= (1 - self.coins[i])
